...
Introducing notation, for each phase of testing we have failure time data for a
given vehicle $i$ and failure mode $j$ denoted as $t_{ij0}, t_{ij1},...,
t_{ijn_{ij}}, t_{ijc}$, where $t_{ij0} = 0$ and $t_{ijc}$ is the end of that
testing phase. In the first phase of testing, we assume the vehicle miles
between failures, $y_{ijk} = t_{ijk} - t_{ijk-1}$, follow a lifetime
distribution with an unknown set of parameters that include a rate parameter
$\lambda_{ij}$.
\begin{equation}
y_{ijk}\mid\lambda_{ij}\sim f(\cdot), \quad i = 1,2,...,v \quad
j=1,2,...,s \quad k=1,2,...,n_{ij}
\end{equation}
where $v$ is the number of vehicles, $s$ is the number of failure modes, and
$n_{ij}$ are the number of failures of vehicle $i$ failure mode $j$. The number
of failure modes is assumed fixed and known \textit{a priori}.
...
distribution.  If failure mode $j$ is considered common across vehicles, the
collection of failure rates is collapsed to a single parameter, $\lambda_{ij} =
\lambda_j$. As with the related failure modes, a prior distribution is placed on
the single failure rate. The prior distributions are independent across failure
...
we define $\lambda_{ij}=(\rho_{j}^{P2})\lambda_{ij}$ where $\rho_{j}^{P2}$
represents the between phase change in failure mode $j$. Given this definition
of of the rate parameter, we again model the miles to failure for a given
vehicle and failure mode using the same lifetime distribution from Test Phase 1.
We will choose a prior distribution for the parameter $\rho_{j}^{P2}$ that has
positive support.  If $\rho_{j}^{P2}$ is less than one, this represents an
improvement in reliability.  After Phase 2 we can look again at failure rates
across failure modes and vehicles and obtain an overall estimate of the rate for
the family of vehicles.  The analysis of Test Phase 3 failure rate
$\lambda_{ij}=(\rho_{j}^{P3})\rho_{j}^{P2}\lambda_{ij}$, following the same
pattern as that shown in Phase 2. At the end of Phase 3, we can look at failure
rates across failure modes and vehicles and obtain an overall estimate of the
rate for the family of vehicles. Future tests will be planned based on the
inferences of Phase 3.
...
Test Phase 2,
\begin{equation}
f(y_{ijk}|\lambda_{ij}\rho_{1,j})=\lambda_{ij}\rho_{j}^{P2}
\exp(-\lambda_{ij}\rho_{j}^{P2} y_{ijk})
\end{equation}
Test Phase 3,
\begin{equation}
f(y_{ijk}|\lambda_{ij}\rho_{j}^{P3}\rho_{j}^{P2})=\lambda_{ij}\rho_{1,j}\rho_{2,j}
\exp(-\lambda_{ij}\rho_{j}^{P3}\rho_{j}^{P2} y_{ijk})
\end{equation}
